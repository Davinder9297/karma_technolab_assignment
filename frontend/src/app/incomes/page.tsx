      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="lg:col-span-4 flex justify-end">
            <button
              type="submit"
              className="bg-blue-600 text-white px-8 py-2.5 rounded-lg font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-100"
            >
              Add & Allocate Income
            </button>
          </div>
            ...
            {lastAllocation.allocations.map((a, i) => (
              <div key={i} className="bg-white p-3 rounded-lg border border-green-100 flex justify-between items-center">
                <div>
                  ...
                  <p className="font-semibold text-slate-700">{a.note === "unallocated" ? "Remaining" : "Bucket Name"}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-green-600">{formatCurrency(a.amount)}</p>
                  <p className="text-[10px] text-slate-400">{a.ruleType === "PERCENTAGE" ? `${a.ruleValue / 1000}%` : "Fixed"}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {incomes.map((income) => (
              <tr key={income._id} className={clsx(
